...
export class MonitoraggioComponent {
  ...
  monitoraggioFormMetabolism: FormGroup;
  ...
  constructor(private fb: FormBuilder, monitoraggio: MonitoraggioService) {
    ...
    this.monitoraggioFormMetabolism = this.fb.group({
      peso: ['', [Validators.required, Validators.min(1)]],
      altezza: ['', [Validators.required, Validators.min(1)]],
      età: ['', [Validators.required, Validators.min(1)]],
      sesso: ['', Validators.required],
  ...
  onSubmitBfp(): void {
    if (this.monitoraggioFormBfp.valid) {
      const { altezza, girovita, collo, fianchi, sesso, peso, età, livelloAttività } = this.monitoraggioFormBfp.value;
      this.bfp = this.calculateBodyFatPercentage(altezza, girovita, collo, fianchi, sesso);
      this.imc = this.calculateBMI(peso, altezza);
      this.bmr = this.calculateBMR(peso, altezza, età, sesso);
      ...
      this.lbm = this.calculateLBM(peso, this.bfp);
      this.ffmi = this.calculateFFMI(this.lbm, altezza);
      this.proteinIntake = this.calculateIdealProteinIntake(peso);
      this.hydrationStatus = this.calculateHydrationStatus(peso);
    }
  }
  ...
  calculateBMR(peso: number, altezza: number, età: number, sesso: string): number {
    let bmr: number;
    if (sesso === 'M') {
      bmr = 88.36 + (13.4 * peso) + (4.8 * altezza) - (5.7 * età);
    } else {
      bmr = 447.6 + (9.2 * peso) + (3.1 * altezza) - (4.3 * età);
    }
  ...
  calculateTDEE(bmr: number, livelloAttività: string): number {
    ...
    const tdee = bmr * (activityMultipliers[livelloAttività] || 1.2);
  ...
  calculateIdealProteinIntake(peso: number): number {
    const proteinIntake = peso * 1.6;
    return parseFloat(proteinIntake.toFixed(2));
  }

  calculateHydrationStatus(peso: number): number {
    const hydrationStatus = peso * 0.6;
